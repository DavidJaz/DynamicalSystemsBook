...
We denote the composite of $\ema{e}\bimod[n]\ema{d}\bimod[m]\ema{c}$ by 
\[\ema{e}\bimod[n\tri_\ema{d}m]\ema{c}.\]
...
\begin{proposition}
For any categories $\cat{C}$ and $\cat{D}$ and $(\cat{D},\cat{C})$-bimodule $\ema{d}\bimod[m]\ema{c}$, we have
\[
m\tri_\ema{c}\ema{c}\cong m
\]
where $\ema{c}\bimod[\ema{c}]\yon$ is the regular left $\ema{c}$-module.
\end{proposition}
...
\begin{proposition}
Every bimodule $\ema{c}\bimod[S]\0$, is isomorphic to the composite
\[
\ema{c}\bimod[S\yon]\yon\bimod[\1]\0
\]
for a bimodule $\ema{c}\bimod[S\yon]\yon$ with carrier $S\yon\in\poly$.

Moreover, $S\yon$ is adjoint to a bimodule $\yon\bimod{\yon^S}\ema{c}$ with carrier $\yon^S\in\poly$.
\end{proposition}
