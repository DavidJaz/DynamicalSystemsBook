...
Let $\cat{C}$ be a category. A pair $(\cat{I},\pi)$, where $\cat{I}$ is a category and $\pi\colon\cat{I}\to\cat{C}$ is a functor, is called a \emph{discrete opfibration over $\cat{C}$} if it satisfies the following condition.
\begin{itemize}
	\item for every object $c\in\cat{I}$, object $d'\in\cat{C}$, and morphism $f\colon \pi(c)\to d'$ there exists a unique object $c'\in\cat{I}$ and morphism $\bar{f}\colon c\to c'$ such that $\pi(c')=d'$ and $\pi(\bar{f})=f$.
\end{itemize}
\[
\begin{tikzcd}
  c\ar[r, dashed, "\bar{f}"]\ar[d, |->, "\pi"']&
  c'\ar[d, |->, "\pi"]\\
  \pi(c)\ar[r, "f"']&
  d'
\end{tikzcd}
\]
A \emph{morphism} $(\cat{I},\pi)\to(\cat{I}',\pi')$ between discrete opfibrations over $\cat{C}$ is a functor $F\colon\cat{I}\to\cat{I}'$ making the following triangle commute:
\begin{equation}\label{eqn.dopf_triangle}
\begin{tikzcd}[column sep=small]
	\cat{I}\ar[dr, "\pi"']\ar[rr, "F"]&&
	\cat{I}'\ar[dl, "\pi'"]\\&
	\cat{C}
\end{tikzcd}
\end{equation}
...
\begin{exercise}
Show that if $F\colon \cat{I}\to\cat{I}'$ is a functor making the triangle \eqref{eqn.dopf_triangle} commute, then $F$ is also a discrete opfibration. 
\end{exercise}

\begin{exercise}\label{exc.dopf_cof}
Suppose $\pi\colon\cat{I}\to\cat{C}$ is a discrete opfibration and $i\in\cat{I}
...
\begin{enumerate}
	\item the category of functors $\cat{C}\to\smset$,
...
Up to isomorphism, discrete opfibrations into $\cat{C}$ can be identified with dynamical systems on $\cat{C}$. This association is only functorial on the groupoid of objects and isomorphisms.
\end{proposition}
\begin{proof}
