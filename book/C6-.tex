...
\end{tabular}
\]
Consider this database state as a functor $S\colon\cat{C}\to\smset$.
...
\end{tikzcd}
\end{equation}
The polynomial map $f$ induces a cofunctor $F\colon S\yon^S\cof\cofree{p}$ from the state category on $S$ to the category of $p$-trees. We can now see this as a database state on $\cofree{p}$, considered as a database schema. 
...
Again, there are infinitely many tables in this schema. Only three of them have data in them; the rest are empty. We know in advance that this state has three rows in total, since $|S|=3$.
\end{example}
...
\begin{exercise}
Give an example of a dynamical system on $p\coloneqq\yon^\2+\yon$ for which the corresponding database state has a table with at least two rows.
\end{exercise}
...
\begin{enumerate}
	...
	\item Explain what the corresponding database state looks like.
	\item In particular, how many total rows does it have?
	\item Give an example with $|S|=7$, displayed both as a dynamical system (with states and transitions) and as a database state.	
\qedhere
\end{enumerate}
\end{exercise}
...
We will see that bimodules in $\poly$ are equivalent to data-migration functors. Given comonoids/categories $\cat{C}$ and $\cat{D}$, a bimodule $\cat{C}\bimodfrom[m]\cat{D}$ is a way of moving database states on $\cat{D}$ to database states on $\cat{C}$. One could call it a ``$\cat{D}$-indexed union of conjunctive queries.''
...
\begin{theorem}\label{thm.tfae_c_sets}
For a comonoid $\com{C}=(\ema{c},\epsilon,\delta)$ (category $\cat{C}$), the following categories are isomorphic:
\begin{enumerate}
	\item functors $\cat{C}\to\smset$;
	\item discrete opfibrations over $\cat{C}$;
	\item cartesian cofunctors to $\com{C}$;
	\item representable right $\com{C}$-modules; and
	\item linear left $\com{C}$-modules;
	\item constant left $\com{C}$-modules;
	\item $(\com{C},0)$-bimodules;
	\item $\com{C}$-coalgebras (sets with a coaction by $\com{C}$).
