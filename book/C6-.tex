...
\begin{example}[Siblings, cousins, second cousins...]
Consider the category $\yon^\nn=\fbox{\begin{tikzcd}\bullet\ar[loop right, "p"]\end{tikzcd}}$. A database instance $\yon^\nn\bimodfrom[S]\0$ on $\yon^\nn$ is a closed discrete dynamical system: a set $S$ and a function $p\colon S\to S$. Here we give an example of a bimodule $\yon^\nn\bimodfrom\yon^\nn$.

There is a query asking for siblings: elements $s_1,s_2\in S$ with $p(s_1)=p(s_2)$. There is also a query asking for first cousins (``second siblings''): elements $s_1,s_2\in S$ with $p(p(s_1))=p(p(s_2))$. And so on: for any $n\in\nn$ we have a query asking for $n$th siblings; when $n=0$ this just returns the elements of $S$. We want a bimodule that returns the disjoint union of all these, but to do so we need to give a function $p$ from that set to itself. This is straightforward: given a pair $(s_1,s_2)$ of $n$th siblings, ask each to return its parent to give the pair $(p(s_1),p(s_2))$.

So given a dynamical system $S$, one obtains a new dynamical system whose states are pairs of eventually colliding $S$-states, where the update function takes each such pair to its one-step evolution, one step closer to the collision.
\end{example}
