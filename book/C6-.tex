...
\end{tikzcd}
\]
Let $P\coloneqq\{r,g,b\}^{\ord{100}\times\ord{100}}$ be a set whose elements we call images, let $W\coloneqq\List(\{\text{a},\ldots,\text{z}\})$ be a set whose elements we call words, and let $T\coloneqq\{\text{Color,Object,Place}\}$ be a set whose elements we call types. We can thus create a database instance $J\colon\Cat{Sq}\to\smset$ sending
\[
  J(\Set{Type})\coloneq T,\quad
  J(\Set{Word})\coloneqq W\times T,\quad
  J(\Set{Picture})\coloneqq P\times T,\quad
  J(\Set{Card})\coloneqq P\times W\times T
\]
