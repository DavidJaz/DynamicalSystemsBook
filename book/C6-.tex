...
\begin{itemize}
	\item for every object $c\in\cat{I}$, object $d'\in\cat{C}$, and morphism $g\colon \pi(c)\to d'$ there exists a unique object $c'\in\cat{I}$ and morphism $f\colon c\to c'$ such that $\pi(c')=d'$ and $\pi(f)=g$.
\end{itemize}
\[
\begin{tikzcd}
  c\ar[r, dashed, "f"]\ar[d, |->, "\pi"']&
  c'\ar[d, |->, "\pi"]\\
  \pi(c)\ar[r, "g"']&
  d'
\end{tikzcd}
