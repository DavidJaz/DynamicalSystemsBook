...
%------------ Chapter ------------%
\chapter[Applications of bimodules in $\smcat^\sharp$]{Applications of bimodules in $\smcat^\sharp$:\\dynamical systems and databases}\label{chapter.6} 
...
In the previous chapter we saw that comonoids in $\poly$ are categories, but that morphisms of comonoids are not functors; they're called cofunctors. If someone were to ask ``which are better, functors or cofunctors'', the answer is clear. Functors are fundamental to mathematics itself, relating branches from set theory to logic to algebra to measure theory, etc. Cofunctors don't have anywhere near that sort of reach in terms of applicability. Still they provide an interesting way to compare categories, as well as invariants like the monoid of direction fields on a category.

In this chapter we'll consider another kind of morphism between categories, one that is a bit more familiar, namely the bimodules between comonoids. One might want to call them bi-co-modules, but this name is just a bit too long and the name bimodule is not ambiguous, so we'll go with it.

It turns out that bimodules between comonoids in $\poly$ (categories) are also important objects of study in category theory. If $\cat{C}$ and $\cat{D}$ are categories, Richard Garner showed that a bimodule between them can be identified with what's known as a \emph{parametric right adjoint} between the associated copresheaf categories $\cat{C}\set$ and $\cat{D}\set$. These come up in $\infty$-category theory, but they also have a much more practical usage: they are the so-called \emph{data migration functors}.

Indeed, we'll make due on a claim we made in \cref{chapter.4}, that there is a strong connection between databases and bimodules between polynomial comonads. Databases have two parts: they have a schema, a specification of various types and relationships between them, and an instance, which is actual data sorted into those types and having those relationships. As we'll see, one can formalize the schema as a category $\cat{C}$ and the instance as a functor $I\colon\cat{C}\to\smset$. Here's an example of a theorem we'll prove:
...
\end{enumerate}
\end{theorem}
The proof will be given in \cref{**}.
...
Let $\cat{C}$ be a small category. One of the most important constructions in category theory is that of the category of copresheaves (or presheaves) on $\cat{C}$. This is the category
\[
...
\begin{example}
Suppose $(G,e,*)$ is a monoid (e.g.\ a group). In a first course on abstract algebra, one encounters the notion of a \emph{$G$-set}, which is a set $X$ together with a $G$ action: for every element $g\in G$ we get a function $\alpha_g\colon X\to X$; we might write $\alpha_g(x)$ as $g\cdot x$. Actions need to satisfy two rules: $e\cdot x=x$ and $g\cdot(h\cdot x)=(g*h)\cdot x$. A morphism between $G$-sets is a function $f\colon X\to Y$ such that $f(g\cdot x)=g\cdot (f(x))$ for all $g\in G$ and $x\in X$.

Now recall that any monoid can be understood as a category with one object, let's call our category $\cat{G}$ and the unique object $\Delta$. Then it turns out that $G$-sets are precisely functors $F\colon\cat{G}\to\smset$: the set $F(\Delta)$ is our $X$ above, and since the elements of $G$ are now morphisms $g\colon \Delta\to\Delta$, they are sent to functions $F(g)\colon X\to X$. The axioms of a functor---preservation of identities and compositions---ensure the two rules of $G$-sets. Finally, morphisms between $G$-sets are exactly the natural transformations between functors.

\slogan{$G$-sets are copresheaves on $\cat{G}$, the associated one-object category.}
\end{example}

\begin{example}[Clue game]
There is a game that is played in the US, called Clue, where the players are detectives trying to solve a murder mystery. The game consists of a board together with a deck of several cards, and on each card there is both some printed words and a picture. For one card, the words might say ``Ms. White'' and it shows a picture of a woman. Every card has a type: it is either a room card, a weapon card, or a person card; e.g.\ there is a card that says ``knife'' and show a picture of a knife.
...
\]
A functor $D\colon\cat{W}\to\smset$ would be a possible Clue deck: a set of cards, each of which has an associated picture, some associated words, and the words and picture both have the same type (e.g.\ room, weapon, person, motive). The particulars of the Hasbro Clue deck $D$ are not important for us; we are interested in the structure they have created.

So let's write out our own $D$ in the form of a database:
\[
d
\]
