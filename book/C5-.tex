...
\begin{tikzpicture}[polybox, tos]
	...
	%
	\draw (s_pos) to[first] node[below] {$\mathrm{get}$} (t_pos);
	\draw (t_dir) to[last] node[above] {$\mathrm{put}$} (s_dir);
\end{tikzpicture}
\]
...
\end{tikzpicture}
\]
It says that $\mathrm{put}(\mathrm{get}(s))=s$. This is typically called the get-put law.
...
\begin{example}
Let $p\coloneqq\{\dot[red],\dot[blue]\yon^\2$
\[
START: tree pic 
\]
\end{example}
