...
\end{tikzpicture}
\]
The idea of this diagram is that each wire carries an exposed variable of the
...
\begin{tikzpicture}[unoriented WD, spacing=8pt, font=\tiny]
	...
	%
	\node[link] at (1.5,0) (t1) {};
	\node[link] at (-.75,1.3) (t2) {};
	\node[link] at (-.75,-1.3) (t3) {};
	%
  \draw (f1) -- (t1);
  \draw (f1) -- (t2);
  \draw (f1) -- (t3);
...
Consider, for example, a thermostat controlling a heating unit that warms a
room. If the temperature of the room falls
below $15{}^{\circ}C$, the heater is turned on and the temperature rises; but if the
temperature exceeds $25{}^{\circ}C$, the thermostat will be turned off and
the temperature will slowly fall. Without paying too much heed to the actual
rate of increase and decrease of the temperature (let's imagine for the moment
that they are linear), we can model this system as a
composite system with components the heater $\Sys{H}$ and the thermostat $\Sys{T}$.




\subsection{Undirected wiring diagrams as spans in categories of arities}\label{sec:behavioral.diagrams}
