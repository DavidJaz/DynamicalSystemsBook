...
A goal we set for ourselves at the beginning of this section (\ref{subsec.working_with_composites}) was as follows. Given a position $k\in (p\tri q)\tri\1$, we have a device to get a position $\trunc(k) \in p\tri\1$. We know that we should then get a function $p_i\to q(\1)$, assigning a position in $q$ to each direction in $p_i$. How do we grab it?

First, form the pullback as described above
\[
\begin{tikzcd}
	\yon\ar[ddr, bend right=15pt, "k"']\ar[drr, bend left=15pt, "!"]\ar[dr, dashed]\\[-10pt]&[-10pt]
	\yon^{p_i}\tri q\ar[d]\ar[r]&
	\1\ar[d, "\trunc k"]\\&
	p\tri q\ar[r, "\trunc"']&
	p\tri \1\ar[ul, phantom, very near end, "\lrcorner"]
\end{tikzcd}
\]
We then compose the induced map $\yon\to\yon^{p_i}\tri q$ with the level-2 trunctation to obtain
\[
\yon\to \yon^{p_i}\tri q\To{\trunc^2}\yon^{p_i}\tri(q\tri\1)\cong\smset(p_i,q(\1))
\]
...
Cofunctors $\cat{C}\to\cat{D}$ are the same thing as morphisms of comonoids $\com{C}\to\com{D}$, so we elide the difference.
...
\begin{proposition}
For any category $\cat{C}$, there is an equivalence of categories between that of functors $\cat{C}\to\smset$ and that of discrete opfibrations over $\cat{C}$.
\end{proposition}
