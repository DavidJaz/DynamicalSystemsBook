...
\[
\begin{tikzpicture}[polybox, font=\footnotesize, shorten <=3pt, shorten >=3pt]
	\node[poly, dom] (p) {};
	\node[left=4pt of p.one] {$p[-]$};
	\node[left=4pt of p.two] {$p(\1)$};
	\foreach \i in {1,...,4}
	{
  	\node[poly, cod] (q\i) at (3,1.3*\i-1.3) {};
  	\node[right=8pt of q\i.one] {$q_{\i}[-]$};
  	\node[right=8pt of q\i.two] {$q_{\i}(\1)$};
	};
	\draw[->] (p.two east) -- node[below] {$f_1$} (q1.two west);
	...
	\draw[->] (q4.one west) to[out=180, in=90] node[above left] {$f^\sharp$} (p.one north);
\end{tikzpicture}
\]
...
\end{gather*}
In the more verbose box-notation we have:
\[
\begin{tikzpicture}[polybox, node distance=.9cm]
  \node (a) {
  \begin{tikzpicture}
  	\node[poly, dom] (p) {$S$\nodepart{two}$A$};
  	\node[poly, cod, right=of p] (q) {$U$\nodepart{two}$C$};
  	\node[poly, cod, above=.3 of q] (r) {$Y$\nodepart{two}$E$};
  	\draw[|->] (p.two east) -- (q.two west);
  	\draw[|->] (q.one west) to[climb] (r.two west);
  	\draw[|->] (r.one west) to[sharp] (p.one north);
  \end{tikzpicture}
  };
  \node[right=of a] (b) {
  \begin{tikzpicture}
  ...
  \begin{tikzpicture}
  	...
  	\node[poly, strike, above=.3 of q] (r) {\nodepart{two}$F$};
  	\draw[|->] (p.two east) -- (q.two west);
  	\draw[|->] (q.one west) to[climb] (r.two west);
  \end{tikzpicture}
  };
...
What if I give you a two-step decision to make. I'll give you a position in $p$, you choose a direction there, and then based on your answer I'll give you a position in $q$, and you choose a direction there too. Now if each of $p$ and $q$ knew how to delegate decisions to its partner, say $p\to p'$ and $q\to q'$, you should be able to delegate your two-step decision in $p\tri q$ to a two-step decision by the partners. We've already discussed this in \cref{subsec.monoidal_struc_tri}, but now we want to see it again in our new notation.

\begin{example}[$\tri$ on morphisms]
Given $f\colon p\to p'$ and $g\colon q\to q'$ we want to talk about $h\coloneqq(f\tri g)\colon (p\tri q)\to (p'\tri q')$. Writing $f=(f_1,f^\sharp)$ and $g=(g_1,g^\sharp)$, we want to write $h$ in the form $(h_1,h_2,h^\sharp)$. 

For an example to keep in mind, recall that in \cref{ex.circ_prod_on_morphisms} we had
\[
\begin{tikzpicture}[rounded corners]
	\node (p) [draw, blue!50!black, "$p=$" left] {
	\begin{tikzpicture}[trees, sibling distance=5mm]
...
Let's prepare by understanding our input polynomial $p\tri q$. Using \cref{ex.pos_in_composite}, a position in $p\tri q$ is a pair $(i,j)$, where $i\in p(\1)$ and $j\colon p[i]\to q(\1)$. A direction in $(p\tri q)[(i,j)]$ is a pair $(d,e)$ where $d\in p[i]$ and $e\in q[j(d)]$.

In our example we had morphism $f\colon p\to p'$ and $g\colon q\to q'$ shown here:
\[
\begin{tikzpicture}
	\node (p1) {\raisebox{.3cm}{$f\colon p\to p'$}\qquad
	\begin{tikzpicture}[trees, sibling distance=2.5mm]
...
We are now ready to write out the components $h_1$, $h_2$, and $h^\sharp$ of $h=f\tri g$:
\[\arraycolsep=3pt
\begin{array}{cccc}
	h_1\colon&((i,j)\in(p\tri q)(\1))&\to& p'(\1)\\
					 & (i,j)              &\mapsto&f_1(i)
\end{array}
\]
We will replace $h_1(i,j)$ below with $f_1(i)$ for simplicity. Here is $h_2$:
\[\arraycolsep=3pt
\begin{array}{cccccc}
	h_2\colon&((i,j)\in(p\tri q)(\1))&\to&(d'\in p'[f_1(i)])&\to&q'(\1)\\
	&(i,j)&\mapsto&d'&\mapsto& g_1(j(f_i^\sharp(d')))
\end{array}
\]
And finally here is $h^\sharp$:
\[\arraycolsep=3pt
\begin{array}{cccccccc}
	h^\sharp\colon&((i,j)\in(p\tri q)(\1))&\to&(d'\in p'[f_1(i)])&\to&(e'\in q'[h_2(i,j,d')])&\to&\displaystyle\sum_{d\in p[i]}q[j(d)]\\\\
	&(i,j)&\mapsto&d'&\mapsto&e'&\mapsto&\big(f_i^\sharp(d'),g_j^\sharp(e')\big)
\end{array}
\]
%Here's a picture of what we've been talking about, namely $f\tri g$:
%\begin{equation}\label{eqn.pic_tri_map}
%\begin{tikzpicture}[decoration={markings, mark=at position 0.75 with \arrow{stealth}}, baseline=(j)]
%	\node (i) {$p[i]$};
%	\node[above=-2pt of i] (tr1) {$\bigtriangledown$};
%	\node[above=-2pt of tr1] (j) {$q[j]$};
%	\node[right=2 of i] (i') {$p'[f_1(i)]$};
%	\node[above=-2pt of i'] (tr') {$\bigtriangledown$};
%	\node[above=-2pt of tr'] (j') {$q'[f_i^\sharp\then j\then g_1]$};
%	\draw[|->] (i) -- (i');
%	\draw[bend right=30, densely dotted, postaction={decorate}] 
%		(i') to 
%			node[above] (fs) {$f_i^\sharp$}
%		(i);
%	\draw[bend right=60, densely dotted, postaction={decorate}] 
%		(j') to 
%			node[above] (gs) {$g_j^\sharp$}
%		(j);
%\end{tikzpicture}
%\end{equation}
%We invite the reader to see \eqref{eqn.pic_tri_map} as a schematic for what we drew as follows:
The above should make sense in terms of our picture of $f\tri g$, shown below.
\[
	\begin{tikzpicture}[trees]
	\begin{scope}[
		level 1/.style={sibling distance=8mm},
...
%
  ...
  \end{tikzpicture}
\]
Each of the six pictures is for a specific $(i,j)$-pair. On the bottom it does $f_1$ to the given $i$; on the middle it does $f_i^\sharp$, then the given $j$, then $g_1$; and on the top it applies $g_j^\sharp$ over $f_i^\sharp$.
\end{example}



\begin{exercise}
Draw a picture analogous to \eqref{eqn.pic_tri_map} for the map $(f\tri g\tri h)\colon (p\tri q\tri r)\to (p'\tri q'\tri r')$, given $f=(f_1,f^\sharp)\colon p\to p'$, $g=(g_1,g^\sharp)\colon q\to q'$, and $h=(h_1,h^\sharp)\colon r\to r'$.
\end{exercise}
