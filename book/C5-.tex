...
We discussed how $\tri$ acts on morphisms in \cref{subsec.monoidal_struc_tri}. Now let's see how it looks in our new notation.

Given $f\colon p\to p'$ and $g\colon q\to q'$ we want to talk about $h\coloneqq(f\tri g)\colon (p\tri q)\to (p'\tri q')$. Writing $f=(f_1,f^\sharp)$ and $g=(g_1,g^\sharp)$, we want to write $h$ in the form $(h_1,h_2,h^\sharp)$, and do so in terms of $f$ and $g$.
...
We are now ready to write out $h_1$, $h_2$, and $h^\sharp$. 
\[\arraycolsep=3pt
\begin{array}{cccc}
...
\begin{tikzpicture}[decoration={markings, mark=at position 0.75 with \arrow{stealth}}, baseline=(j)]
	...
	\node[above=-2pt of tr1] (j) {$j$};
	\node[right=2 of i] (i') {$f_1(i)$};
	\node[above=-2pt of i'] (tr') {$\bigtriangledown$};
	\node[above=-2pt of tr'] (j') {$f_i^\sharp\then j\then g_1$};
	\draw[|->] (i) -- (i');
	\draw[bend right=60, densely dotted, postaction={decorate}] 
		(j') to 
			node[above] (fs) {$\tristack{g_j^\sharp}{f_i^\sharp}$}
		(j);
\end{tikzpicture}
\end{equation}
For example, in \cref{ex.circ_prod_on_morphisms} we had
\[
\begin{tikzpicture}
	\node (p1) {\raisebox{.3cm}{$f\colon p\to p'$}\qquad
	\begin{tikzpicture}[trees, sibling distance=2.5mm]
...
%
  ...
  \end{tikzpicture}
\]
Each of the six pictures is for a specific $(i,j)$-pair. On the bottom it does $f_1$ to the given $i$; on the middle it does $f^\sharp$, then the given $j$, then $g_1$; and on the top it applies $g^\sharp$ over $f^\sharp$.
\end{example}



\begin{exercise}
Draw a picture analogous to \eqref{eqn.pic_tri_map} for the map $(f\tri g\tri h)\colon (p\tri q\tri r)\to (p'\tri q'\tri r')$, for $f=(f_1,f^\sharp)\colon p\to p'$, $g=(g_1,g^\sharp)\colon q\to q'$, and $h=(h_1,h^\sharp)$.
\end{exercise}
