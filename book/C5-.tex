...
Let $S\coloneqq\3$ and $p\coloneqq\yon^\2+\yon$, and consider the dynamical system $f\colon S\yon^S\to p$ from \cref{exc.det_fsa_misc_398}, depicted here for your convenience:
\[
\begin{tikzcd}[column sep=small]
	\LMO{1}\ar[rr, bend left, orange]\ar[loop left, dgreen]&&
	\LMO{2}\ar[dl, bend left, orange]\ar[ll, dgreen, bend left]\\&
	\LMO{3}
\end{tikzcd}
\]
...
The answer is that we associate to each state its tree of possible futures. For example, to the state $1$ we associate the tree
\[
\treepic
\]
Let's call it $t$. We will say how $F$ acts on the other two states in \cref{exc.cofree_dyn_sys}, but before doing so, let's say how $F$ passes back directions. A direction $d\in\tree_p[t]$ is just a finite path, say of length $n$, out of the root. Each arrow $d\in p[\pi(t)]$ out of the root is passed back to a new state $\cod(f^\sharp(d))\in S$. This new state maps to the codomain of $d$, so we can repeat this procedure $n$ times.
\end{example}

\begin{exercise}\label{exc.cofree_dyn_sys}
Let's finish off \cref{ex.cofree_dyn_sys}.
\begin{enumerate}
	\item Where does the cofunctor $F$ send the state $1$?
	\item Where does the cofunctor $F$ send the state $1$?
\qedhere
\end{enumerate}
\end{exercise}
