...
%---- Subsection ----%
\subsection{Working with composites}

A position $i\colon\1\to p\tri q$ in the composite $p\tri q$ consists of a choice of position in $p$ and, for every direction there a position in $q$. How do we grab the $p$-position? And then given such a $p$-position, how do we talk about having an additional direction there with which to grab a $q$-position? We need to have tools for saying such things---or even more complicated things about morphisms $p\tri q\to p'\tri q'$ carefully. We give such tools in this section.

Recall that $\1$ is the terminal object in $\poly$, so we have a unique morphism $!_q\colon q\to\1$. Since $\tri$ is monoidal, we have the following useful function
\[
p\tri q\To{p\tri !_q}p\tri\1.
\]
So given $i$ as above, we can compose with this map and get $i\then(p\tri !_q)\colon\1\to p\tri 1=p(\1)$, the position in $p$ we wanted.

\begin{exercise}
Let $p,q$ be as in \cref{ex.circ_prod_on_morphisms}, let $p'\coloneqq p$ and $q'\coloneqq 1$, and let $f\coloneqq\id_p$ and $g\coloneqq !_q$.
\begin{enumerate}
	\item Draw $f\tri g$ in the style of that exercise.
	\item Would you say it ``grabs the positions of $p$ and nothing more'', or is that subtly inaccurate somehow?
\qedhere
\end{enumerate}
\end{exercise}

More generally, given polynomials $q_{1},\; q_{2},\;\ldots\; q_{n}$,
%\footnote{Notation is as in \cref{prop.coprod_prod_poly}: we put a comma after each subscript, e.g.\ $q_{1}$ to indicate that it is a whole polynomial, not a set of directions.}
we may want to truncate their composite tree at height $0\leq m\leq n$; this operation is denoted
\[
  \trunc^m\colon q_{1}\tri q_{2}\tri\cdots\tri q_{n}\to 
  q_{1}\tri q_{2}\tri\cdots\tri q_{m}
\]
The $0th$ truncation is the unique morphism $\trunc^0\colon q_{1}\tri q_{2}\tri\cdots\tri q_{n}\to\1$ to the terminal object $\1$. 

Given a morphism $f\colon p\to (q_{1}\tri\cdots\tri q_{n})$, we denote the composite of $f$ with the $m$th truncation by
\[\trunc^m(f)\colon p\to q_{1}\tri\cdots\tri q_{m}.\]
In particular, we can think of a position as a map from $\yon$, so given $i\in q_{1}\tri\cdots\tri q_{n}(\1)$, we write $\trunc^m(i)\in q_{1}\tri\cdots\tri q_{m}(\1)$ for its $m$th truncation.

We denote the first truncation simply by $\trunc=\trunc^1$.

\begin{warning}
Note that $\trunc^m$ is not an operation on polynomials! It is an operation on expressions involving composites, e.g.\ $p\tri q\tri r$, etc. Two isomorphic polynomials, expressed differently as composites, may yield different results when hit by $\trunc^m$; we'll see an example of this in \cref{exc.truncation}.

We call $\trunc^m$ the \emph{$m$-truncation device}, the word device being meant to signal that it's not functorial.
\end{warning}

\begin{exercise}\label{exc.truncation}
\begin{enumerate}
	\item Find polynomials $p,q,r$ such that $r\cong (p\tri q)$.
	\item Is $\trunc(p\tri q)$ the same map as $\trunc(r)$? If ``no'', explain why; if yes, repeat all over again from the beginning of step 1 and choose something less degenerate.
\qedhere
\end{enumerate}
\end{exercise}

\begin{exercise}
Suppose given a morphism $f\colon p\to q\tri r$.
\begin{enumerate}
	\item How do you grab the on-positions map $p\tri\1\to (q\tri r)\tri 1$? That is, what operation can you use to obtain it from $f$?
	\item How do you grab the function that takes a $p$-position $i\in p(\1)$ and returns a $q$-position; is it just $f_1(i)$?
	\item Get the function $p\tri\1\to q\tri\1$ from $f$ using only the truncation device $\trunc$ and the functor $-\tri\1$.
\qedhere
\end{enumerate}
\end{exercise}
...
\begin{example}
Suppose we have polynomials $p$ and $q$ and a position $i\in p(\1)$. How do we grab that part of the composite $p\tri q$? That is, how do we find the Cartesian subpolynomial
\[
  \prod_{d\in p_i}\;\sum_{j\in q(\1)}\;\prod_{e\in q_j}\;\yon
  \ss
...
\begin{tikzcd}[background color=examplecolor]
	\yon^{p_i}\tri q\ar[d]\ar[r]&
	\1\tri q\ar[r, equal]\ar[d, "i\tri q"]&
	\1\ar[d, "i"]\\
...
A goal we set for ourselves at the beginning of this section was as follows. Given a position $k\in (p\tri q)\tri\1$, we have a device to get a position $\trunc(k) \in p\tri\1$ and know that for every direction $d\in p_{\trunc(k)}$ there, we are supposed to get a position in $q$. How do we grab it?

Well, by the cartesian closure of $\poly$ (see \cref{thm.poly_cart_closed,eqn.eval_times}), we have an evaluation map
\[
\text{eval}\colon X\times\yon^X\to\yon
\]
for any set $X$. In particular, we have $p_{\trunc(k)}\times\yon^{p_{\trunc(k)}}\to\yon$. Let's write $p_k\coloneqq p_{\trunc(k)}$.

So suppose we are given both $k\in (p\tri q)\tri \1$ and $d\in p_k$. Using the pullback \eqref{eqn.cart_pullback_misc48} $k$ induces a map $k'\colon\1\to\yon^{p_k}\tri q$ and we have an induced element $(d,k')$ in $p_{\trunc k}\yon^{p_k}\tri q\tri\1$; see \cref{ex.connected_limits_and_tri}. Thus we can evaluate:
\[
p_k\yon^{p_k}\tri q\tri\1
\to
\yon\tri q\tri\1
\cong
q\tri\1.
