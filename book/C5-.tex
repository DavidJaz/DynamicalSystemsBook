...
\begin{example}[$\delta^{n}$ notation]\label{••}
...
Let $p\coloneqq\{\bul[red],\bul[blue]\}\yon^\2+\bul[dgreen]\yon+\bul[dyellow]$. Here are four $p$-trees:
...
The way the adjunction works is as follows. Suppose that $\cat{C}=(\ema{c},\epsilon,\delta)$ is a comonoid and $p$ is a polynomial. Then given a map of polynomials $f\colon\ema{c}\to p$, we can form a map $\ema{c}\to p\tripow{k}$ for any $k\in\inn$ as follows. First use the comonoid structure to obtain $\delta^{k-1}\colon\ema{c}\to\ema{c}\tripow{k}$ as in \cref{ex.delta_n_notation}. Second we use the monoidality of $\tri$ to get $f\tripow{k}\colon\ema{c}\tripow{k}\to p\tripow{k}$. Composing we obtain
\[
...
Discuss counit map $\pi\colon\tr_p\to p$, the map sending each $p$-tree to its \emph{root corolla} and each direction there back to the one-step path.
