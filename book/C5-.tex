...
\begin{remark}
The construction of the cofree comonoid from a copointed endofunctor in the proof of \cref{thm.cofree} is fairly standard; see \cite{lack2010note}. Nelson Niu has also constructed the cofree comonoid using a different limit diagram
\[
\begin{tikzcd}
	\yon\ar[d]&
	p\ar[d]&
...
\end{tikzcd}
\]
in terms of the original polynomial $p$, rather than from its copointing. One could also use the following limit, again from the original polynomial $p$:
\[
\begin{tikzcd}[column sep=small]
	\yon\ar[dr]&&
	p\ar[dl]\ar[dr]&&
