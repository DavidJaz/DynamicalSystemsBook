...
%
	\node [blue, right=2 of A2] (B1) {$\bullet$}
...
%
	\node [dgreen, right=2 of B2] (C1) {$\bullet$}
...
Now we draw $(\delta\tri c)\circ\delta$ and $(c\tri\delta)\circ\delta$ and see that they disagree at the $A$ position. First we draw $(\delta\tri c)\circ\delta$ which does $\delta$ first, then copies the 
\[
\begin{tikzpicture}[trees, bend right=60,
level 1/.style={sibling distance=5mm},
level 2/.style={sibling distance=2.5mm}]
