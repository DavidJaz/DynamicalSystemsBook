...
Do you follow along with the fact that $2*14=28$ and $2*28=56$ and $2*56=114$, and that when you carry the one you get $57$ instead? That's a pattern, right? Indeed, each pattern has a certain shape or repetitiveness, like the tree you see here:
\[
...
\end{tikzpicture}
\]
Do you follow along with the fact that the tree sitting at every green dot is the same as every other one, and same with yellow and red? That's a pattern, right?

When someone says ``there's a pattern in my life'', they're talking about something that recurs, something with a certain kind of \emph{shape}. If they pick up on something again and again, it's a pattern alright. 
