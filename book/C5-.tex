...
\end{theorem}
\begin{proof}
This will be proved in \cref{??}.
\end{proof}

\paragraph{Constructing comonoids from categories}

Given a category $\cat{C}$ with notation as in \cref{def.cofunctor}, we will first construct a corresponding comonoid $\com{C}\coloneqq(\ema{c},\epsilon,\delta)$. We usually denote objects of $\cat{C}$ by $i,j$ rather than $c$ so as not to confuse with $\ema{c}$. 
...
\begin{align*}
  \sum_{m\in C_i}C_{\cod(m)}&\To{\delta^\sharp_i}C_i\\
...
It remains to check the unitality and associativity.
