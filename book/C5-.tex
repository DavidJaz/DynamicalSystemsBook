...
Let $p\in\poly$ be a polynomial. The comonoid $\cofree{p}=(\tr_p,\rt,\fs)$ as in \cref{thm.cofree} is called the \emph{cofree comonoid on $p$} or informally the category of \emph{trees in $p$}.

An object $t\in \tr_p\tri\1$ is a called a \emph{(possibly infinite) tree in $p$.} An emanating morphism $n\in\tr_p[t]$ is called a \emph{node in $t$}.
\end{definition}
