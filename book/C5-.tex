...
\paragraph{Constructing categories from comonoids.}

Given a comonoid $\com{C}\coloneqq(c,\epsilon,\delta)$, we now construct a corresponding category $\cat{C}$. Its objects are given by $C_0\coloneqq c(\1)$ and its morphisms are given by $\sum_{i\in C_0}(c_i)$. The domain of each morphism is $\dom(i,m)\coloneqq i$. The identities will be given by $\epsilon$, the codomains will be given by $\delta_1$, and the composites will be given by $\delta^\sharp$.
