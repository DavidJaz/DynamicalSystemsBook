...
\begin{tikzpicture}[polybox, mapstos]
	...
	\draw (p'_dir) to[sharp] node[above] {$i^\sharp$} (yX_dir);
\end{tikzpicture}
\]
...
\begin{tikzpicture}
  ...
  \begin{tikzpicture}[polybox, tos]
  	...
  	\draw (c2_dir) to[sharp] node[above] {$\delta^\sharp$} (c_dir);
  	\draw (c1_pos) -- (id_pos);
...
\end{tikzpicture}
\]
We'll put the other unitality picture in the following exercise, and then get to the associativity in a little while. The meaning of $\epsilon$ and $\delta$ will become clear; for those who want a hint, see \cref{***}.
\begin{exercise}
...
\subsection{Understanding comonoids}
...
\begin{tikzpicture}
  ...
  %
  	...
  	\draw (c2_dir) to[sharp] node[above] {com} (c_dir);
  \end{tikzpicture}
	};
	\node[right=2 of 1] (2) {
  \begin{tikzpicture}[polybox, tos]
...
\[
  ...
  %
  	...
  	\draw (c1_dir) to[climb] node[left] {$\cod$} (c2_pos);
  	\draw (c2_dir) to[sharp] node[above] {$\comp$} (c_dir);
  \end{tikzpicture}
\]
...
\end{tikzpicture}
\]
Ah, it's saying that it wants $\cod(m')=\cod(m\then m')$; well yeah, that's how codomains should work. And it wants $m\then(m'\then m'')=(m\them m')\then m''$, classic associativity. Amazing; thanks giant brain!
