...
\subsection{Polynomial comonoids are categories}\label{subsec.understanding_comonoids}
...
\end{tikzpicture}
\]
The $\epsilon$ map is saying that the identity on $s$ is $s$. Remember that both the set of objects and the set of morphisms emanating from any given object are $S$. The map $\cod=\delta_2$ is telling us that the codomain of a map $s_1$ is the object $s_1$, and that the composite of $s_1$ and $s_2$ is $s_1\then s_2=s_2$. 

What this all means is that $\cat{S}$ is the category with $S$-many objects and a unique morphism $s\to s'$ for any $s,s'\in S$. Here's a picture for $S=\3$ and $S=\1\5$, with all maps (even identities) drawn:
\[
\begin{tikzpicture}
\def\n{3}% how many nodes
\def\size{2cm}
...
Anyway, to avoid confusion, we'll refer to $\cat{S}$ as the \emph{state category on $S$}, because we use these to think about states of dynamical systems, and also because the state comonad in functional programming is $S\yon^S$.
\end{example}
...
\begin{enumerate}
	...
	\item How many categories are there that have just one linear and one quadradic object?
...
\begin{enumerate}
	\item Consider the chain poset $[n]\cong\sum_{i=1}^n\yon^i$. How many cofunctors are there from $[m]\to[n]$ are there for all $m,n\in\{0,1,2,3\}$?
...
\begin{exercise}
\begin{enumerate}
	\item What is the finite set $\{\com{Q}_1,\ldots,\com{Q}_n\}$ of comonoids for which the carrier polynomial is $\yon^\2+\yon$?
...
\begin{example}
There is a cofunctor $S\yon^S\otimes T\yon^T\to S\yon^S$, and every cofunctor between state categories is of this form.
\end{example}
...
Let $\cat{C}$ be a kategory, $A$ a set, and $f\colon A\to\Ob(\cat{C})$ a function to the objects of $\cat{C}$. There is an induced kategory $f^*\cat{C}$ with objects $A$ and cofunctor $f^*\cat{C}\to\cat{C}$ that is universal in the appropriate sense. Said more precisely, we have the following proposition.
...
The following says that cofree kategories are very special objects, much like posets.
...
\subsection{$\otimes$ structure on $\smcat^\sharp$}

The usual product of categories gives a monoidal operation on comonoids; the carrier polynomial of the product is the $\otimes$-product of the carrier polynomials.

\begin{proposition}[$\otimes$ monoidal structure on $\Cat{Comon}(\poly)$]
Let $\com{C}=(\ema{c},\epsilon,\delta)$ and $\com{D}=(\ema{d},\epsilon,\delta)$ be comonoids. The polynomial $\ema{c}\otimes\ema{d}$ has a natural comonoid structure, which we denote $\com{C}\otimes\com{D}$. This construction gives a monoidal structure $(\yon,\otimes)$ on $\smcat^\sharp$.
\end{proposition}
\begin{proof}
**
\end{proof}
...
Said another way, a \emph{monoidal kategory} is a monoid object $(\cat{C},\intercal, \curlyvee)$ in the monoidal category $(\Cat{Comon}(\poly), \otimes,\yon)$.
\end{definition}
