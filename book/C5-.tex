...
\slogan{Imagine a sort of realm, where there are various positions you can be in. From every position, there are a number of moves you can make, possibly infinitely many. But whatever move you make, you'll end up in a new position. Well, technically it counts as a move to simply stay where you are, so you might end up in the same position. But wherever you move to, you can move again, and the any number of moves from an original place counts as a single move. What sort of realm is this?}
...
\begin{example}[Trajectories are co-representable]\label{ex.trajectories_corep}
...
\[
\adj{\Cat{Comon}(\poly)}{U}{\bar{q}}{\Cat{Copnt}(\poly)}.
\]
\end{theorem}
...
\treepic
\]
Let's call it $t$. We will say how $F$ acts on the other two states in \cref{exc.cofree_dyn_sys}, but before doing so, let's say how $F$ passes back directions. A direction $d\in\tr_p[t]$ is just a finite path, say left-left-right, coming out of the root. Each arrow $d\in p[\pi(t)]$ out of the root is passed back to a new state $\cod(f^\sharp(d))\in S$. This new state maps to the codomain of $d$, so we can repeat this procedure $n$ times.
\end{example}

\begin{exercise}\label{exc.cofree_dyn_sys}
Let's finish off \cref{ex.cofree_dyn_sys}.
\begin{enumerate}
	\item To what tree does the cofunctor $F$ send the yellow state?
	\item To what tree does the cofunctor $F$ send the red state?
\qedhere
\end{enumerate}
\end{exercise}

\begin{exercise}
Let $S\coloneqq\4$ and $p\coloneqq\{\bul[red],\bul[dgreen],\bul[dyellow]\}\yon$, and let $f\colon S\yon^S\to p$ be the dynamical system depicted as follows:
\[
\begin{tikzpicture}
	\node[draw] {
  \begin{tikzcd}
  	\bul[red]\ar[r, bend left]&
  	\bul[dgreen]\ar[d, bend left]\\
  	\bul[red]\ar[u, bend left]&
  	\bul[dyellow]\ar[l, bend left]
  \end{tikzcd}
  };
\end{tikzpicture}
\]
Let $s\in S$ be the state for which $f_1(s)=\bul[dgreen]$. 

If $F\colon S\yon^S\cof\cofree{p}$ is the associated cofunctor, what is the tree $F_1(s)$?
\end{exercise}

\begin{exercise}
Suppose $S$ is a set, $p$ is a polynomial, $f\colon S\yon^S\to p$ is a dynamical system, and $F\colon S\yon^S\cof\cofree{p}$ is the associated cofunctor. To every state $s\in S$, we get a tree $F(s)\in\tr_p$. What common feature do all such trees have when $S$ is a finite set? What do these trees look like?
\end{exercise}

\begin{exercise}
In \cref{ex.counting_trajectory} we considered a dynamical $\nn\yon^\nn\to\nn\yon$ that acts like a stop watch, ticking up for ever.
\begin{enumerate}
	\item Obtain a map of polynomials $\nn\yon^\nn\to\yon$ from adjointness of the cofree construction.
	\item Obtain a map of polynomials $\nn\yon^\nn\to\nn$.
	\item Does the pairing of the two maps you found have the stopwatch behavior we were looking for?
\qedhere
\end{enumerate}
\end{exercise}

The way the adjunction works is as follows. Suppose that $\cat{C}=(\ema{c},\epsilon,\delta)$ is a comonoid and $p$ is a polynomial. Then given a map of polynomials $f\colon\ema{c}\to p$, we can form a map $\ema{c}\to p\tripow{k}$ for any $k\in\nn$ as follows. First use the comonoid structure to obtain $\delta^{k-1}\colon\ema{c}\to\ema{c}\tripow{k}$ as in \cref{ex.delta_n_notation}. Second we use the monoidality of $\tri$ to get $f\tripow{k}\colon\ema{c}\tripow{k}\to p\tripow{k}$. Composing we obtain
\[
\ema{c}\To{\delta^{k-1}}\ema{c}\tripow{k}\To{f\tripow{k}}p\tripow{k}
\]
This is a generalization of what we called ``speeding up dynamical systems'' in \cref{eqn.speedup}. In terms of poly-boxes it looks like this for $k=3$:
\begin{equation}\label{eqn.comon_delta_k}
\begin{tikzpicture}
	\node (given) {
	\begin{tikzpicture}[polybox, tos]
		\node[poly, dom, blue, "$\ema{c}$" left] (S) {};
		\node[poly, cod, dgreen, right=of S, "$p$" right] (p) {};
		\draw (S_pos) to[first] (p_pos);
		\draw (p_dir) to[last]  (S_dir);
		\node at ($(S.east)!.5!(p.west)$) {$f$};
...
%
  ...
  \end{tikzpicture}	
	...
	\node[above] at (obtain.north) (obtain_lab) {Obtain for $k=3$};
	\node at (given|-obtain_lab) {Given $f\colon \ema{c}\to p$};
\end{tikzpicture}
\end{equation}
So for any $k\in\nn$, we have a map from $\ema{c}$ to $p$-trees of height $k$, and we put them together to obtain the map $\cat{C}\to\cofree{p}$. The fact that we have an adjunction $\adj{\smcat^\sharp}{}{}{\poly}$ means that there is nothing more or less in a cofunctor $\cat{C}\to\cofree{p}$ than the map $\ema{c}\to p$ of polynomials.

It is also worthwhile understanding the unit and counit of this cofree adjunction. The counit is a map of polynomials $\tr_p\to p$ natural in $p\in\poly$. On positions, it sends a tree $t$ to its root corolla. Given a direction in the root corolla, we obtain a direction in $t$, i.e.\ a path (of length 1) emanating from the root.

\begin{example}
Let's consider $p=\{\bul[red],\bul[blue]\}\yon^\2+\bul[dgreen]\yon+\bul[dyellow]$ as in \cref{ex.imagining_trees}. Given a $p$-tree $t$ that starts like this:
\[
\begin{tikzpicture}[trees,
  level 1/.style={sibling distance=10mm},
  level 2/.style={sibling distance=5mm},
...
\end{tikzpicture}
\]
So the map $\tr_p\to p$ sends $t$ to $\bul[red]$. It passes each of the two directions emanating from $\bul[red]$ to the corresponding direction emanating from the root of $t$.
\end{example}

\begin{exercise}
Let $p\coloneqq\yon^\2+\1$.
\begin{enumerate}
	\item Draw a $p$-tree, i.e.\ an element $t\in\tr_p(\1)$.
	\item What is the root corolla of $t$?
	\item To what position does the map $\pi\colon\tr_p\to p$ send $t$?
	\item For each direction in $p[\pi(t)]$, show the direction in $\tr_p[t]$ that it is passed back to by $\pi^\sharp$.
\qedhere
\end{enumerate}
\end{exercise}

Given a comonoid $\cat{C}=(\ema{c},\epsilon,\delta)$, the unit cofunctor $\cat{C}\to\cofree{\ema{c}}$ sends each position (object $c\in\cat{C}$) to the tree that puts at the root the set of all morphisms emanating from $c$, to each such morphism associates its codomain and then the set of morphisms emanating from that, and so on. The passback map simply takes a composable sequence of morphisms and composes them.
