...
A \to \powset A$ defined by $\mu = \id_{\poweset A}^{\powset}$ sends a set $S$
...
costs are finite.
  \item Given sets $A$ and $B$, the cost of having an element of $A$ and an
    element of $B$ should be the sum of their costs. But we are forced by the
    laws of a commutative monad to set it to be the minimum of their
    costs: $$\sigma(c, c')(a, b) \coloneqq \min\{c(a), c'(b)\}.$$
  \end{itemize}
\end{definition}
