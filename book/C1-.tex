...
\[
<<<<<<< HEAD
\begin{tikzpicture}[line cap=rect,line width=3pt, baseline=center]
=======
\begin{tikzpicture}[line cap=rect,line width=3pt, baseline=(bl)]
\coordinate (bl) at (0,0);
>>>>>>> 691333f6ae713ac84963a90f48c339787ae26ef8
\filldraw [fill=cyan] (0,0) circle [radius=2cm];
\foreach \angle [count=\xi] in {60,30,...,-270}
{
  \draw[line width=1pt] (\angle:1.8cm) -- (\angle:2cm);
...
\begin{exercise}\label{ex.find_steady_states1}
  Write down the steady state matrices of systsm $\Sys{S_1}$ and $\Sys{S_2}$ from
...
\begin{definition}[Periodic orbit] \label{def.periodic_orbit_discrete}
  ...
  exists a time $t_0 \in \nn$ such that $s_{t_0} = s_0$.   
  The \emph{period} of a periodic trajectory is the smallest positive $t_0$ so
  that $s_{t_0} = S_0$. \end{definition}

\begin{remark}
  Note that a steady state is a periodic orbit with a period of $0$.
\end{remark}
