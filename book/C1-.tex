...
\[
  ...
  \end{tikzcd}
\]
in the double category of \emph{arenas}, \cref{def.double_category_of_arenas}.
...
chapter, the particular notion of lens and chart might change, but they will
...
\ctxto A^_$ in $\smctx{A^+}$, which means that $f^{\sharp}$ is a really a
function $A^+ \timse B^- \to A^_$. In other words, a map in $\int^{C \in \smset}
