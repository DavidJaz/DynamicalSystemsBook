...
diagrams looks like. We'll calculate the following system $\Sys{S}$ from definitions of
$\Sys{S_1}$ and $\Sys{S_2}$ below.
\[
\Sys{S} = 
\begin{tikzpicture}[oriented WD, bbx = .3cm, bby =.3cm, bb min width=.5cm, bb port length=2pt, bb port sep=1, baseline=center]
...
The fully wired system $\Sys{S}$ will then have 12 states
