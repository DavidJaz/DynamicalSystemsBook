...
\subsection{Non-deterministic and stochastic systems}
...
\end{tikzpicture}
\]
The inputs to this system are from the set $\In{S} = \{\Green, \Orange\}$, and
the outputs are from the set $\Out{S} = \{a, b\}$. Suppose that we can only see
what the system is outputing, and that it is outputing a $b$.
