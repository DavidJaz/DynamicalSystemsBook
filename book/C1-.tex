...
\begin{tikzpicture}[oriented WD, bbx = .3cm, bby =.3cm, bb min width=.5cm, bb port length=2pt, bb port sep=1, baseline=(Boston.center)]
...
\begin{definition}
  ...
  \begin{itemize}
    \item Objects $\prod_{\tau \in \cat{T}} \XX_{\tau}^{I_\tau}$ for any typed
      family finite sets $I_{(-)} : \cat{T} \to \Cat{Fin}$.
    \item Arrows $f^{\ast} : \prod_{\tau \in \cat{T}} \XX^{I_\tau} \to
      \prod_{\tau \in \cat{T}} \XX^{J_\tau}$ for any family of functions
      $f_{\tau} : J_\tau \to I_{\tau}$ indexed by the types $\tau \in \cat{T}$.
    \item Comopsition is given by $g^{\ast} \circ f^{\ast} = (f \circ g)^{\ast}$
      where the family $f \circ g$ is defined pointwise by $(f \circ g)_\tau =
      f_\tau \circ g_\tau$. The identity is given by $\id := \id^{\ast}$. 
  \end{itemize}
  ...
  of types. We interpret the family $I_{(-)} : \cat{T} \to \Cat{Fin}$ as the
  projection map $\sum_{\tau \in \cat{T}} I_\tau \to \cat{T}$. 
\end{definition}

\begin{exercise}
  We blew through that isomorphism $\arity_{\cat{T}} \cong (\Cat{Fin} \downarrow
  \cat{T})\op$ quickly, but its not trivial. Expand this out in full and check
...
universal property.
\begin{proposition}\label{prop.arity_univeral_property_typed}
...
\begin{corollary}\label{cor.arity_change_of_types}
  ...
  $\ev{\XX_f} \arity_{\cat{T}} \to \arity_{\cat{T}}$
\end{corollary}

\begin{example}
  We can describe the wiring diagram $\lens{w^{\sharp}}{w}$ in \cref{ex.ClockWithDisplay} as a lens in
  ...
  \end{align*}
  giving us a wiring diagram in $\Cat{WD}_{\cat{T}}$
\end{example}
