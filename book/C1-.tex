...
\begin{exercise}
  ...
  \cat{T})\op$ quickly, but its not trivial. The category $\Cat{Fin} \downarrow
...
\subsubsection{Indexed categories and the Grothendieck construction}
...
$\lens{A}{C} \leftrightarrows \lens{A}{C}$; this will be come clear in
...
\begin{definition}
  Let $\cat{A} : \cat{C}\op \to \Cat{Cat}$
\end{definition}




\subsubsection{CoKleisli categories and maps with context}\label{sec.context_indexed_cat}
  

\Section{Trajectories, Steady States, and Periodic Orbits: Maps of Deterministic Systems}  \ref{sec.trajectories_discrete}
