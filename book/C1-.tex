...
We now need to interpret our wiring diagrams as lenses. The diagram should
...
\leftrightarrows \lens{\XX^{B^{-}}}{\lens{XX^{A^+}}}$ in $\Cat{Arity}$ as a
wiring diagram:
\begin{itemize}
  ...
  \item Then we remember that $w^{\ast} :
    \XX^{A^+} \to \XX^{B^+}$ comes from a reindexing function $w : B^+ \to A^+$)
