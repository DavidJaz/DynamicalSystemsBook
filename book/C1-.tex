...
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(X.center)]
...
\[
\begin{tikzpicture}[line cap=rect,line width=3pt, baseline=(0,0)]
\filldraw [fill=cyan] (0,0) circle [radius=2cm];
\foreach \angle [count=\xi] in {60,30,...,-270}
{
  \draw[line width=1pt] (\angle:1.8cm) -- (\angle:2cm);
...
those input wires:
\begin{equation}\label{eqn.diner_system_box}
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(Outer.center)]
...
\begin{equation}\label{eqn.diner_system_box}
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.3cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(Outer.center)]
	\node[bb={1}{1}] (X1) {$\Sys{a.m./p.m.}$};
  	\node[bb={0}{1}, below=2 of X1] (X2) {$\Sys{Clock}$};
...
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(Tape.center)]
...
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(Box.center)]
...
\begin{tikzpicture}[oriented WD, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=2pt, bb port sep=1, baseline=(Outer.center)]
  \node[bb={1}{2}](Box) {$\Sys{Mystery Box}$};
  \node[bb={2}{2}, right=of Box] (Tape) {$\Sys{Tape Machine}$};
  
  \node[bb={0}{0}, fit={($(Tape.south east) + (0, -2)$) ($(Box.north west) + (-1, 1)$)}] (Outer) {};
...
\begin{tikzpicture}[oriented WD, bbx = .3cm, bby =.3cm, bb min width=.5cm, bb port length=2pt, bb port sep=1, baseline=(Sys.center)]
...
\begin{tikzpicture}[oriented WD, bbx = .3cm, bby =.3cm, bb min width=.5cm, bb port length=2pt, bb port sep=1, baseline=(Outer.center)]
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item Arrows $f^{\ast} : \prod_{i \in I} \XX_{\tau_i} \to
...
\section{Mapping Systems: Trajectories, Steady States, and Periodic Orbits}  \label{sec.trajectories_discrete}
...
study the behavior of our dynamical systems. We will define the notions of
trajectory, steady state, and periodic orbit of deterministic systems, and see
all of these as examples of \emph{maps} of deterministic systems.

A map $m$ from
$\Sys{S}$ to $\Sys{T}$ is a function $m : \State{S} \to \State{T}$ which
preserves the dynamics of the systems invovled (see
\cref{def.system_map_discrete} for the full definition). Maps are so useful
because they allow us to find ``$\Sys{S}$-shaped parts of the system
$\Sys{T}$''. For example, a map from the clock system $\Sys{Clock}$ to a system
$\Sys{S}$ will be sequence of states that loop back to the beginning after
twelve steps.
...
The calculation will turn out to be just a bit of matrix arithmetic! But that's
...
\begin{definition}
 ...
 \emph{$(p, v)$-trajectory} is a sequence of states $s : \nn \to \State{S}$ so
 that
 \begin{align*}
   \update{S}(s_i, p_i) &= s_{i + 1}\\
   \expose{S}(s_i) &= v_i
 ...
 All in all, we have a family of trajectories for the system $\Sys{S}$, indexed by pairs of input streams
 and output streams:
 $$\Set{Traj}(\Sys{S}) \to \In{S}^{\nn} \times \Out{S}^{\nn}.$$
\end{definition}
...
Suppose that $p : \nn \to \{{\color{dgreen} green}, {\color{orange} orange}\}$
alternates between ${\color{dgreen} green}$ and ${\color{orange} orange}$. Then
...
\begin{exercise}
 ...
 \begin{enumerate}
   \item $p_{2t} = {\color{dgreen} green}$ and $p_{2t + 1} = {\color{orange} orange}$.
   \item $p_t = {\color{dgreen} green}$.
   \item $p_0 = {\color{dgreen} green}$ and $p_t = {\color{orange} orange}$ for
     all $t > 0$.
 \end{enumerate}
\end{exercise}
...
\begin{definition}\label{def.steady_state_discrete}
 ...
 $o \in \Out{S}$, an \emph{$(i,0)$-steady state} is a state $s \in \State{S}$
 such that
 \begin{align*}
   \update{S}(s, i) &= s, \\
   \expose{S}(s) &= o.
 \end{align*}
 All in all, we have a family of steady states indexed by input output pairs:
 $$\Set{SteadyState}(\Sys{S}) \to \In{S} \times \Out{S}.$$
\end{definition}
...
state is steady for the input parameter ${\color{orange} orange}$ but not for
the input parameter ${\color{dgreen} green}$.
\end{remark}
...
the steady states into a $\In{S} \times \Out{S}$ matrix. For example, in
\cref{ex.transition_diagram_discrete}, we get the following $\{{\color{dgreen}
  green},\, {\color{orange} orange}\} \times \{a,\, b\}$ matrix:
\begin{equation}\label{eqn.steady_state_matrix_transition_diagram_discrete}
\kbordermatrix{
  & {\color{dgreen} green} & {\color{orange} orange}\\
...
\section{CoKleisli categories and the Grothendieck construction: Another way
  ...
  \begin{enumerate}
    ...
    \item The notion of a \emph{map} between dynamical systems --- which
      ...
      --- is best formalized by a related construction \cref{def.double_cat_interface_discrete} which uses the
...
\subsection{Indexed categories and the Grothendieck construction}
...
\begin{definition}\label{def.grothendieck_construction}
  ...
  \begin{itemize}
    ...
    \item Arrows $\lens{f_{\flat}}{f} : \lens{A}{C} \rightrightarrows
    ...
    \item Given $\lens{f_{\flat}}{f} : \lens{A}{C} \rightrightarrows
      ...
      $$\lens{g_{\flat}}{g} \circ \lens{f_{\flat}}{f} := \lens{\gamma_{g, f}
        \circ f^{\ast}g_{\flat}
...
\begin{proposition}[Functoriality of the Grothendieck construction]\label{prop.groth_construction_functoriality}
  ...
  $$\lens{\overline{F}}{F} \lens{A}{C} := \lens{\overline{F}A}{FC}.$$
\end{proposition}
\begin{proof}
