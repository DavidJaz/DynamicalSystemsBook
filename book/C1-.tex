...
\begin{informal}
  A \emph{behavior} of a dynamical system is a particular way its states could
...
In \sec{sec.behaviors}, we will give a formal definition of behavior of
...
\begin{equation}\label{eqn.map_discrete}
\begin{aligned}
  \expose{S}(\phi(t)) &= f(\expose{T}(t)), \\
  \update{S}(\phi(t), f_{\flat}(t, i)) &= \phi(\update{T}(t, i))
\end{aligned}
\end{equation}
...
\cref{eqn.map_discrete} becomes the equations:
\begin{align*}
...
\]
  ...
  $\Sys{S}$. Then, \cref{eqn.map_discrete} becomes the equations
  \begin{align*}
    \expose{S}(s) &= o\\
    \update{S}(s, i) &= s
  \end{align*}
...
sequence of states of $S$, and \cref{eqn.map_discrete} become the equations
\begin{align*}
  \expose{S}(\gamma(t)) &= v(t) \\
...
\begin{definition}\label{def.category_of_charts}
  ...
  \begin{itemize}
    ...
    \item Composition the composite of a chart $\lens{f_{\flat}}{f} : \lens{A^-}{A^+}
      \rightright \lens{B^-}{B^+}$ with a chart $\lens{g_{\flat}}{g} : \lens{B^-}{B^+}
      \rightright \lens{C^-}{C^+}$ is 
