...
$\update{}$ and $\expose{}$ maps are continuous gives rise to a determinist
...
\begin{proposition}
  The Euler method gives rise to a change of doctrine
  \[
\mathcal{E}_{\epsilon} : \eucdiff_{|\textbf{Aff}} \to \determ_{\Cat{Euc}}.
  \]
  This is given by
  ...
  $$\phi(s, v) = s + \epsilon \cdot v.$$
\end{proposition}
\begin{proof}
  We note, first of all, that composing with $\phi$ gives us the correct formula
  for the Euler approximation. All that we need to show is that $\phi$ is a
...
\[
f(s + \epsilon \cdot v) = f(s) + \epsilon Tf(s, v)
\]
which says that incrementing $s$ by $\epsilon$ in the $v$ direction in $f$ is
the same as incrementing $f(s)$ by the $\epsilon$ times the directional
...
into the above formulas for the Euler method, then we find that the equation
only holds up to $O(\epsilon^2)$. For affine functions, this means that it does
...
duality between charts and lenses.
...
\begin{itemize}
\item For a $\dd_1$-arena $\lens{A^-}{A^+}$,
  we have the functor 
  \[
    \sigma\littlelens{\phi}{\id}^{\littlelens{A^-}{A^+}} :
    \Cat{Sys}_{\dd_2}\littlelens{A^-}{A^+} \to \Cat{Sys}_{\dd_2}\littlelens{\overline{F}A^-}{FA^+}.
  \]
...
\[
    ...
    \end{tikzcd}
\]
\item For a $\dd_1$-chart $\lens{f_{\flat}}{f} : \lens{A^-}{A^+} \tto \lens{B^-}{B^+}$,
