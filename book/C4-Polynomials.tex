...
\begin{agda}
record Interface : Set where  -- an interface consists of 
   field                      -- two fields
     pos : Set                -- one called pos, a set, and
     dis : pos -> Set         -- one called dis, that depends on pos
\end{agda}  
...
\begin{proposition}[Yoneda lemma]\label{lemma.yoneda}
Given a functor $F\colon\smset\to\smset$ and a set $S$, there is an isomorphism
\begin{equation}\label{eqn.yoneda}
F(S)\cong\Hom(\yon^S,F)
\end{equation}
where $\Hom$ denotes the set of natural transformations. Moreover, \eqref{eqn.yoneda} is natural in both $S$ and $F$.
\end{proposition}
...
\begin{enumerate}
	...
	\item As a corollary of \cref{lemma.yoneda}, show that $\yon^-\colon\smset\op\to\smset^\smset$ is fully faithful, in particular that there is an isomorphism $\Hom(\yon^S,\yon^T)\cong S^T$.
\qedhere
\end{enumerate}
\end{exercise}
...
\end{equation}
$\rr$ denotes the set of real numbers, $\zz$ denotes the set of integers, $\3$ denotes the set $\{1,2,3\}$, and $A$, $I$, $Q_i$, and $R_i$ denote some arbitrary sets that should have already been defined in order for \eqref{eqn.crazy_poly} to make sense. But what are $+$ and $\sum$?

The polynomials we already understand at this point are the pure-power polynomials $\yon^A$ for some set $A$; they are functors $\yon^A\colon\smset\to\smset.$
So to understand general polynomials like $p$, we just need to understand sums of functors. Sum is another name for \emph{coproduct}. It will be useful to discuss products of functors at the same time. However, we haven't discussed sums and products of sets yet, so we need to do that first.
...
\begin{definition}[Sums and products of sets]
...
\begin{exercise}
When $I=\varnothing$, there is a unique collection $X\colon I\to\smset$; it is empty, i.e.\ it consists of no sets. What is the coproduct and product of this collection?
\end{exercise}


\begin{definition}[Sum and product of functors $\smset\to\smset$]\label{def.sum_prod}
...
\begin{exercise}\label{exc.zero_sums_and_prods}
Let $I\coloneqq\varnothing$ and $X\colon \varnothing\to\smset$ the unique empty collection of sets.
\begin{enumerate}
	\item Show that there is an isomorphim of sets $\0\cong\sum_{i\in\varnothing}X_i$.
...
\end{proposition}
\begin{proof}
By \cref{ex.two_sums_and_prods,exc.zero_sums_and_prods}, it suffices to show that $\sum_{i\in I}F_i$ and $\prod_{i\in I}F_i$ are a sum and product in $\smset^\smset$. This itself is a special case of a more general fact, where sums and products are replaced by arbitrary colimits and limits, and where $\smset^\smset$ is replaced by an arbitrary functor category $\cat{C}^\cat{D}$, where $\cat{C}$ is a category that (like $\smset$) has limits and colimits; see \cite[page 22 -- 23, displays (24) and (25)]{macLane1992sheaves}.
\end{proof}

So we've finally arrived: we can define the category of polynomial functors.

\begin{definition}[Polynomial functors]
A \emph{polynomial functor} is a functor $p\colon\smset\to\smset$ such that there exists a set $I$, sets $(p_i)_i\in I$, and an isomorphism
