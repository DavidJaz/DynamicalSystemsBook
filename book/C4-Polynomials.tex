...
In \cref{exc.file_reader} one is tasked with making a file reader, where a file is a function $f\colon\ord{n}\to\Set{ascii}$. Now we take that same idea but make the robot have a different interface when it is in read-mode: namely, one where it cannot take in signals.
...
\]
as follows. On positions define
\[
	r_1(s,t)\coloneqq
	\begin{cases}
