...
\begin{equation}\label{eqn.dir_hom}
	[q,r]\coloneqq\prod_{j\in q(\1)}r\circ(q[j]\yon)
\end{equation}
...
To investigate this just a bit, note that $[p,\yon]\cong\prod_{i\in p(\1)}p[i]\yon$. That is, for each position in $p$ it produces a direction there, which is just what $p$ needs as input.
...
\footnote{
We put a comma in the subscript of our polynomials, writing $p_{a,}$ rather than $p_a$ for the $a$th polynomial, to avoid a potentially ambiguous notation. That is, if $i\in p(\1)$ is a position, then $p[i]$ denotes the set of directions there, the exponent of the $i$th summand of $p$. Given a position $i\in p_{a,}$ we denote the corresponding set of directions by $p_{a,i}$. We recognize that our notation $p_{a,}$, while unambiguous, is a bit disorienting; luckily, it will not come up much.
}
Their coproduct is
