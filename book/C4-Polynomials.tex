...
After that interlude, you are hopefully more comfortable with \cref{eqn.poly_maps_prod_sum} describing the set of morphisms between two arbitrary polynomial functors. Let's practice.

\begin{exercise}
Let $p,q$ be polynomials as in \cref{prop.poly_maps_colax_prod_sum}. Is it true that the following are isomorphic?
\[
  \prod_{i\in p(\1)}\sum_{j\in q(\1)}{p_i}^{q_j}
  \cong^?
  \prod_{i\in p(\1)}\sum_{j\in q(\1)}\prod_{d\in q_j}\sum_{c\in p_i}\1
\]
If so, say why; if not say why not.
\end{exercise}
...
\begin{proposition}
There is a fully faithful functor $\smset\to\poly$ sending $S\in\smset$ to the constant polynomial $S\yon^\0$, which we also denote by $S$.
