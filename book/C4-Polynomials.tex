...
%------------ Chapter ------------%
\chapter{Polynomial functors and dynamics}\chapter{chapter.4}
...
The category $\poly$ has limits and colimits and is Cartesian closed. Let's start off with something that fairly simple.
...
\end{proposition}
\begin{proof}
Let $A$ be a set and $p\colon A\to\poly$ a collection of polynomials $(p_a)_{a\in A}$. Their coproduct is
...
\begin{enumerate}
	...
	\item Show that \eqref{eqn.prod_polys} satisfies the universal property for coproduct of polynomials.
\qedhere
\end{enumerate}
\end{exercise}
...
\end{theorem}
\begin{proof}
Recall that a category has all limits iff it has equalizers and products, so by \cref{prop.coprod_prod_poly} it suffices to show that $\poly$ has equalizers. Let $f,g\colon p\tto q$ be two maps of polynomials. Let $E\inj p(\1)\tto q(\1)$ be an equalizer of the functions $f(\1),$ and $g(\1)$ in $\smset$. For each $e\in E$Define $p'$ as follows
\[
  p'\coloneqq\sum_{e\in E}\yon
\]
\end{proof}
...
\spiz{Discuss adjunctions with $\smset$ and $\smset\op$, two-variable adjunctions, and adjunctions with pointed sets op. Discuss constant, linear, affine, pure-power polynomials, monomials as lenses.}
