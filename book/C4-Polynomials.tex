...
We can generalize both $S\yon^S$ and $B\yon^A$ in \cref{def.moore_machine_again}, though the latter is much simpler and we'll spend much of the section on it. First let's briefly preview something we're going to spend a lot of time later, namely what's special about $S\yon^S$.
...
\begin{align*}
  \text{get}\colon\rr^2&\to\rr^2&&
  \text{put}\colon\rr^2\times[0,1]\times[0,2\pi]&\to\rr^2\\
  (a,b)\mapsto(a,b)&&
  (a,b,r,\theta)\mapsto(a+r\cos\theta,a+r\sin\theta)
\end{align*}
\end{example}
...
Let's say a file of length $n$ is a function $f\colon\ord{n}\to\2\5\6+\{\text{done}\}$; suppose given such a file and refer to elements of $\ord{n}=\{1,\ldots,n\}$ as positions. Make a robot (Moore machine) whose output type is \2\5\6 and whose input type is
\[
...
When we get to generalized Moore machines, we will be able to let the robot ``close its port'', so that while it is reading it is not receiving signals; see \cref{ex.generalized_file_reader}.

\begin{exercise}[Turing machine]

\end{exercise}

\paragraph{$S\yon^S$ is a comonad on $\smset$.}
...
\begin{exercise}
Let $f\colon S\yon^S\to B\yon^A$ be a Moore machine. Suppose we wanted to be able to pause it. That is, rather than pressing a button in $A$, we want to press a different button, ``pause", that does nothing to the machine. How do we get this pause-able machine from $f$?
\end{exercise}

%---- Subsection ----%
\subsection{Generalized Moore machines}

Everything we've done above was for interfaces of the form $B\yon^A$, i.e.\ monomials. But the theory certainly works just as well for an arbitrary $p$.
...
\paragraph{More general systems}

\begin{example}\label{ex.generalized_file_reader}
In \cref{exc.file_reader} one is tasked with making a file reader. Now we take that same idea but make the robot have a different interface when it is in read-mode: namely, one where it cannot take in signals.
