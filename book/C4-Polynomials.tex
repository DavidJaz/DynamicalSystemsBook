...
In a polynomial like $42\yon^3+17\yon$, the pure power term $\yon^3$ has a coefficient of 42 next to it, so the corolla with three leaves shows up 42 times in the associated poly. Intuitively, there are 42 situations in which you have a decision with three possibilities.
...
Each eyeball in this system will correspond to the polynomial $\yon^n+\yon$, which intuitively consists of two situations, one with $n$ possibilities, and the other with only one possibility.
...
The set of such decision streams in fact form the objects of a category with very nice properties (it's a topos), which we call $\sys(\yon^2+1)$. The idea is that every corolla in these diagrams has either two possibilities, corresponding to $\yon^2$, or no possibilities, corresponding to $1$.

\begin{exercise}
\begin{enumerate}
	\item Draw a level-3 abbreviation of a decision stream of type $\yon^2+1$.
...
What we have been calling polynomials---things like $\yon^2+2\yon+1$---are often called \emph{containers} in the computer science literature. Here is some sample Agda code for something isomorphic to $\yon^2+2\yon+1$:
\begin{agda}
