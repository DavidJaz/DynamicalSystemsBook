...
$$\prod_{a^+ \in A^+} \sum_{b^+ \in B^+} (A^-_{a^+})^{(B^-_{b^+})}.$$
\end{exercise}
...
\begin{align*}
  ...
  \prod_{i\in p(\1)}\sum_{j\in q(\1)}{p_i}^{q_j}&
  	\text{Yoneda lemma}
\end{align*}
This proves the first part of we called the summary. It says that a morphism $p\to q$ consists of: for every $i\in p(\1)$ both a choice of $j\in q(\1)$ and a function from $q_j\to p_i$; the first part determines $f$ (i.e.\ $f(i)=j$) and the second part just gives $f^\sharp_i$ directly for each $i$.

Next, we need to show that the composition of polynomial morphisms corresponds
to the composition of dependent lenses. We leave this to \cref{ex.poly_dependent_lens_functoriality}.
\end{proof}
...
\sum_{j\colon\prod_{i\in I}J(i)}\;\prod_{i\in I}X(i,j(i))
\cong
...
\]
We need to define a functor $f_*\colon\poly/p\to\poly/q$ and prove the analogous isomorphism establishing it as right adjoint to $f^*$. We take the formula for this desired isomorphism and use it to define $f_*$:
\begin{align*}
	(\poly/p)(f^*q', p')&\cong
	\prod_{i\in p(\1)}\;\prod_{\{j'\in q'(\1)\,\mid\,g(j')=f(i)\}}\;\sum_{\{i'\in p'(\1)\,\mid\,e(i')=i\}}\;(q_{fi}/\smset)(p'_{i'},q'_{j'})\\&\cong
	\prod_{j\in q(\1)}\;\prod_{\{j'\in q'(\1)\,\mid\, g(j')=j\}}\;\prod_{\{i\in p(\1)\,\mid\,f(i)=j\}}\;\sum_{\{i'\in p'(\1)\,\mid\,e(i')=i\}}\;(q_j/\smset)(p'_{i'},q'_{j'})\\&\cong
	\prod_{j\in q(\1)}\;\prod_{\{j'\in q'(\1)\,\mid\, g(j')=j\}}\;\sum_{\{i'\in f_*p(\1)\,\mid\,e'(i')= j\}}\;(q_j/\smset)\left((f_*p')_{i'}, q'_{j'}\right)\\&\cong
	(\poly/q)(q',f_*p')
\end{align*}
Thus we define
\[f_*p'(\1)\coloneqq\sum_{j\in q(\1)}\prod_{\{i\in p(\1)\,\mid\,f(i)=j\}}\sum_{\{i'\in p'(\1)\,\mid\,e(i')=i\}}\1.\] 
For each $i'\in f_*p'(\1)$ define $(f_*p')_{i'}$ to be the coproduct in $q_j/\smset$ of \spiz{finish}
