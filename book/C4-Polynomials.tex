...
\[
\Cat{PRA}(\cat{C}\set,\cat{D}\set)\cong\Cat{Bimod}(\com{C},\com{D})
\]
...
A morphism $f\colon p\to q$ of polynomials induces a pre-geometric morphism between their respective toposes
\[
\adj{\sys(p)}{}{}{\sys(q)}
\]
\end{proposition}

If you skipped over any or all of that, no problem at all! We will get to everything in \cref{subsec.math_theory} over the next few chapters. In the mean time, the results we have just seen in particular say that there is a full-fledged logic of dynamical systems in any interface (\cref{prop.sysp_is_topos}), that these logics can be combined and compared (\cref{prop.poly_map_pregeo_topos})
