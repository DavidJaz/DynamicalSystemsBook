...
\begin{theorem}\label{thm.poly_lcc}
Cartesian morphisms in $\poly$ are exponentiable. That is, if $f\colon p\to q$ is cartesian then the functor $f^*\colon\poly/q\to\poly/p$ given by pulling back $q'\to q$ along $f$ is a left adjoint:
\[
...
%\begin{align*}
%	(\poly/p)(f^*q', p')&\cong
%	\prod_{i\in p(\1)}\;\prod_{\{j'\in q'(\1)\,\mid\,g_1(j')=f_1(i)\}}\;\sum_{\{i'\in p'(\1)\,\mid\,e_1(i')=i\}}\;(p_i/\smset)(p'_{i'},p_i+_{q_{fi}}q'_{j'})\\&\cong^{\spiz{??}}
