...
$\In{S}_p$ valid for $p$ to be the directions we can give to the robot: that is
to say, $\In{S}_p \cong S^1$ should form a circle. However, we want these
directions to be directions that the robot could actually travel, so we will let
$\In{S}_p = \{v \in T_p \Out{S}\mid |v| = 1\}$ be the unit circle in the tangent
...
compositionality results about systems. In the next section, we will organize
these results into \emph{doubly indexed functors}, but for now, it will suffice
to understand what they mean on their own.
...
\begin{itemize}
  ...
  \item Its maps are \emph{simulations}, the squares 
\[
    \begin{tikzcd}
      \lens{T\State{T}}{\State{T}} \ar[r, shift left, "\lens{T\phi}{\phi}"] \ar[r, shift right] \ar[d, shift right,
      "\lens{\update{T}}{\expose{T}}"'] \ar[d, shift left, leftarrow] &
