...
$$\overline{\alpha \then \beta} = \bar{\alpha} \mid \bar{\beta}.$$
\end{lemma}
\begin{proof}
What we have written here simply as an equality is not quite accurate; this is
...
\end{tikzcd}
\]
We note that both of these isomorphisms are given by composing two arrows, so we
...
\begin{tikzcd}& & & F_2F_1 X \ar[ddrr,color=red, "F_2\alpha_f"]
  \arrow[dl,color=blue,"F_2\alpha_X \then \beta_{F_1 X}" description] \arrow[dlll, color=red,
  ...
  X & & \\& & & F_2F_1Y\arrow[dl,color=blue,"F_2\alpha_Y \then \beta_{F_1 Y}" description] \arrow[dlll]\arrow[rr] &  & F_2G_1 Y\arrow[dlll,
