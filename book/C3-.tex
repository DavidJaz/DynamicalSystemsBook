...
The Euclidean differential doctrine $\eucdiff$ is a special case of a
\emph{cartesian differential doctrine}. The category $\Cat{Euc}$ is a
\emph{cartesian differential category}, and for any cartesian differential
category we can make a cartesian differential doctrine. We won't define the
notion of 
...
\begin{definition}
  ...
  $T$, we have a doctrine $\cartdiff$ defined by the indexed category $\smctx{-}
...
\subsection{Dependent deterministic doctrines}

The doctrine $\dispdoc_{(\cat{C}, T)}$ of display maps can also help us describe deterministic
doctrines in which the sorts of input a system can accept depend on the output
that system is currently exposing. These are called \emph{dependent} deterministic
systems.

\begin{definition}
Let $(\Ca, \Da)$ be a category with display maps (\cref{defn:display.maps}) and
finite products, and suppose that product projections $\pi_1 : C \times D \to C$
are display maps. Then we have a section $TC = C \times C \xto{\pi_1} C$, and so
we may define the depdendent deterministic doctrine $\depdet_{C}$ to be the
display map doctrine $\dispdoc_{(\cat{C}, \cat{D}), T}$.
\end{definition}
...
\{\const{open}, \const{closed}\}$. When the diner is closed it excepts no input,
...
Dependent deterministic systems in $\smset$ are tremendously interesting in
their own right, and we won't be able to do them justice here.
...
doctrines and saw that they could all be represented by the same equations
...
organize charts and lenses, and finally gave a formal definition of dynamical
system doctrine. 

The notion of dynamical systems doctrine, the double category of arenas in a
given doctrine, and the definition of behavior of system that these 
